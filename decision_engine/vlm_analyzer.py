...
*   **做空门槛（动态要求）**:
    *   最稳妥的情况下，下列信号至少满足两项，再给出明确看跌；
    *   若仅满足一项，但能确认其它来源没有反向看多信号（全部为中性/HOLD），可以给出保守的看跌结论，并在报告中注明“单一空头信号 + 无冲突”；
    *   一旦发现任何明显的多头证据或更高周期强势，必须回到中性/震荡判断。
    *   候选信号：
...
**盈利锁定与动能衰竭（必须遵守）:**
*   本段用于约束“见好就收”：当你识别到动能衰竭时，必须给出明确的锁盈/平仓建议，且**不得**写“等待下次突破/等待再次确认”。
*   **多头动能衰竭（上涨动能耗尽）**：若你判断价格处于上涨末端或冲高受阻，并出现以下任意两项（或你认为证据足够的组合）：
    * RSI顶背离/从高位明显回落；
    * 放量滞涨或创新高时量能不增反降；
    * 连续上冲失败、长上影、冲高回落幅度增大；
    * 价格在上轨附近走弱、跌回布林上轨/中轨附近且均线斜率变平。
    则在“理由”中明确写出：**若已有盈利多单，优先立即平仓/减仓锁定盈利，不等待下一次突破确认**。
*   **空头动能衰竭（下跌动能耗尽）**：若你判断价格处于下跌末端或砸盘后乏力，并出现以下任意两项（或你认为证据足够的组合）：
    * RSI底背离/从低位明显回升；
    * 放量下跌后续跌无量或下破不延续；
    * 连续下探失败、长下影、急跌后快速反抽；
    * 价格触及/跌破下轨后收回、重新站回关键均线/中轨附近。
    则在“理由”中明确写出：**若已有盈利空单，优先立即平仓/减仓锁定盈利，不等待下一次破位确认**。
